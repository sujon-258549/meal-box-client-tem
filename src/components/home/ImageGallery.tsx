...
const ImageGallery = ({
  ...
  return (
    <section className={`py-3 md:py-6 ${className}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 py-8 md:py-14 lg:grid-cols-5 gap-4 md:gap-6">
        {menus?.data?.slice(0, 10).map((menu) => (
          <motion.div
            style={{ boxShadow: "8px 8px 8px" }}
            key={menu._id}
            whileTap={{ scale: 1 }}
          ...
          >
            <img
              ...
              className="w-full h-40 md:h-40 object-cover transition-transform duration-300 group-hover:scale-105"
              loading="lazy"
            />
